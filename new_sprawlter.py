...
def calcNodeNodePenalty(phase, meta_node_info):
    ...
    for i in meta_node_info.keys():
        ...
        # あるmeta nodeと他のmeta nodeとの重なり(NN ペナルティ)を調べる
        for j in range(i+1, len(meta_node_info.keys())):
            ...
            # 2つのメタノード間の重なりの面積を求めたい
            # if a circle encloses another circle
            if dist < diffr:
                # 片方の円が他方の円にすっぽり → 小さい方の円の面積が「重なりの面積」
                if r1 > r2:
                    # 面積のルート
            ...
            # if circles partially overlap
                ...
                cos1 = (dist * dist + r1 * r1 - r2 * r2) / (2 * dist * r1) # 中心間距離と半径のなす角のcos
                ...
                # math.acos(cos1): cos1の角度=中心間距離と半径のなす角
                # r1 * r1 * math.acos(cos1) : 扇型の面積  (TODO角度半分では...？)
    ...
    # 全てのmeta node間での面積の重なりを確認
    # maxnnは記録されていて、p0の更新に反映されている...！
    return penalty
